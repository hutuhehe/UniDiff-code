    ...
    def forward(self, feats_A, feats_B):
        ...
        for layer in self.decoder:
            if isinstance(layer, Block):
                ...
                f_B = torch.cat([feats_B[i * len( self.feat_dims) + scale_idx] for i in range(len(self.time_steps))], dim=1)

                pdb.set_trace()
                # Compute absolute difference between before & after feature maps
                ...
                x = diff  # Store for next level fusion
                scale_idx += 1
