...
# modify on March 4th adding class condition
class FeatureExtractorDDPM(FeatureExtractor):
    ...
    def _load_pretrained_model(self, model_path,fine_tune = False, **kwargs):
        import inspect
        ...
        # if model finetuned ,load finetuned model architecutre and trained parameter
        if  fine_tune:
            self.model = apply_lora_to_unet_model(self.model)
        
        self.model.load_state_dict(
            dist_util.load_state_dict(model_path, map_location="cpu",weights_only=True)
        )
    ...
    def forward(self, x, noise=None, y = None):
        ...
        for t in self.steps:
            ...
            #pdb.set_trace()
            self.model(noisy_x, self.diffusion._scale_timesteps(t),y)
